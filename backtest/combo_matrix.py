...
def signal_counts(sigs: dict, combo: str) -> tuple[int,int]:
    if combo=="TIER":
        frame=sigs["tier"]
        return int(frame["TIER_L"].fillna(False).sum()), int(frame["TIER_S"].fillna(False).sum())
    frame=sigs["1h"] if NATIVE_TF[combo]=="1h" else sigs["15m"]
    return int(frame[f"{combo}_L"].fillna(False).sum()), int(frame[f"{combo}_S"].fillna(False).sum())

def run_variant(d15, sigs, combo, label, commission, policy, ob15=None, ob1=None):
    r=backtest_parallel(
        d15,sigs,enabled=[combo],commission_pct=commission,policy=policy,
        ob15=ob15,ob1h=ob1,
    )
    sl,ss=signal_counts(sigs,combo)
    row={
        "combo":combo,
        "native_tf":NATIVE_TF[combo],
        "variant":label,
        "signals_long":sl,
        "signals_short":ss,
        "signals_total":sl+ss,
...
def main():
    ...
    ap.add_argument("--variants",default="RAW,OB0,OB05",
                    help="RAW,OB0,OB05. RAW is always recommended for calibration first.")
    ...
    ob_cache={}
    if "OB0" in variants:
        cfg=OBConfig(danger_atr=0.0)
        ob_cache["OB0"]=(ob_context(d15,cfg),ob_context(d1,cfg))
    if "OB05" in variants:
        cfg=OBConfig(danger_atr=0.5)
        ob_cache["OB05"]=(ob_context(d15,cfg),ob_context(d1,cfg))
    ...
    for combo in combos:
        ...
        for variant in variants:
            if variant=="RAW":
                row,r=run_variant(d15,sigs,combo,"RAW",args.commission,args.policy)
            elif variant in ob_cache:
                ob15,ob1=ob_cache[variant]
                label="OB_0.0ATR" if variant=="OB0" else "OB_0.5ATR"
                row,r=run_variant(d15,sigs,combo,label,args.commission,args.policy,ob15,ob1)
            else:
                raise ValueError(f"Unknown variant: {variant}")
            row["symbol"]=args.symbol
            row["data_first_15m"]=str(d15.index.min())
            row["data_last_15m"]=str(d15.index.max())
            rows.append(row)

            if not r["trades"].empty:
                r["trades"].to_csv(combo_dir/f"{variant}_trades.csv",index=False)
            if not r["vetoes"].empty:
                r["vetoes"].to_csv(combo_dir/f"{variant}_vetoes.csv",index=False)

    summary=pd.DataFrame(rows)
    cols=["symbol","combo","native_tf","variant","signals_long","signals_short","signals_total",
          "trades","wins","losses","win_rate","profit_factor","net_pnl","max_drawdown",
          "ending_equity","vetoes","data_first_15m","data_last_15m"]
