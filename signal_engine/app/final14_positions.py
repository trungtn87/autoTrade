...
def register_execution(state, signal, result: dict) -> dict:
    """Persist one active FINAL14 position per symbol+combo.

    A BingX-accepted but not-yet-resolved entry is kept active (position_id may
    be empty) so a later event cannot create a duplicate position.
    """
...
def _resolve_unassigned(rec: dict, positions: list[dict], claimed: set[str]) -> str:
    ...
    for pos in positions:
        ...
        qty0 = float(rec.get("qty") or 0)
        px0 = float(rec.get("avg_price") or 0)
        if qty0 <= 0 or px0 <= 0:
            # Accepted order with an unresolved fill: only resolve if exactly
            # one unclaimed position exists on the required side.
            candidates.append((0.0, 0.0, 0.0, pid))
            continue
    ...
    if float(rec.get("qty") or 0) <= 0 or float(rec.get("avg_price") or 0) <= 0:
        return candidates[0][3] if len(candidates) == 1 else ""
...
def refresh_symbol(state, executor, symbol: str) -> dict:
    """Reconcile persisted FINAL14 cases against BingX positions.

    Call this only when the symbol has a new signal. Normal no-signal scans do
    not make position API requests.
    """
    rows = _load(state)
    ...
    for rec in rows:
        ...
        # Fail closed for an accepted-but-unresolved entry: keep the combo
        # active rather than risk a duplicate position.
        kept.append(rec)
