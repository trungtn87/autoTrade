...
def entry_variants(pc, selected=None):
    """Evaluate a requested name per combo, or the full grid for research.

    Filtering happens before series evaluation. The default research path and
    each selected expression retain their original formulas and edge timing.
    """
    def wanted(combo, name):
        return selected is None or selected.get(combo) == name

    d15=pc["d15"]; d1=pc["d1"]; d4=pc["d4"]; d6=pc["d6"]; p15=pc["15"]; p1=pc["1"]
    out={}

    if selected is None or any(NATIVE[c] == "1h" and c != "TIER" for c in selected):
        # ----- 1H base components -----
        ...
        for bm in [1.0,1.2,1.4,1.6,1.8]:
            if not wanted("C6", f"BODY_{bm:.1f}"):
                continue
    ...
    for bm,vm in itertools.product([1.3,1.5,1.6,1.8],[1.3,1.5,1.8,2.0]):
        if not wanted("C7", f"BODY{bm:.1f}_VOL{vm:.1f}"):
            continue
    ...
    if selected is None or "C8" in selected:
    ...
    if selected is None or "C9" in selected:
        ...
        for rth in [35,40,45]:
            if not (wanted("C9", f"RSI{rth}_ORIGST") or wanted("C9", f"RSI{rth}_FIXST")):
                continue
    ...
    for adxth in [18,20,22,25,28]:
        if not wanted("C10", f"ADX_{adxth}"):
            continue
    ...
    if selected is None or "TIER" in selected:
        ...
        for adxth,t2,t3,vm in itertools.product([23,25,26,28,30],[7,8,9],[5,6,7],[1.3,1.5,1.8]):
            if not wanted("TIER", f"ADX{adxth}_T2{t2}_T3{t3}_V{vm:.1f}"):
                continue
